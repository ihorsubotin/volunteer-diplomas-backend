...
export class TelegramController {
	...
	async generateNewConnection(@Req() req) {
		const connection = await this.telegramService.generateConnection(
			req.user.id,
		);
		const url = this.config.get('TELEGRAM_START_URL') + connection.connectToken;
		return { url };
	}
	...
	async getConnection(@Param() params, @Req() req) {
		...
		if (connection?.user.id === req.user.id) {
			const success = await this.telegramService.deleteUserConnection(
				params.id,
			);
			if (success) {
	...
	async loginViaToken(@Req() req, @Body() body: LoginDTO) {
		const connection = await this.telegramService.validateConnection(
			body.token,
		);
		if (!connection) {
			throw new UnauthorizedException();
		}
		const oldUser = await this.telegramService.getUserByTelegram(
			body.telegramUser,
		);
		if (oldUser) {
		...
		await this.telegramService.saveConnection(
			connection,
			body.userInfo,
			body.telegramUser,
		);
	...
	async generateLink(@Body() generateLink: TelegramUserDto) {
		const user = await this.telegramService.getUserByTelegram(
			generateLink.telegramUser,
		);
		if (!user) {
