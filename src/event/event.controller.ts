...
export class EventController {
	...
	async create(@Body() createEventDto: CreateEventDto, @Req() req) {
		...
		let previousEvent = await this.eventService.getEventWithParticipants(
			createEventDto.previousEvent,
		);
	...
	async findTelegramParticipate(
		@Param('page', ParseIntPipe) page: number,
		@Body() body: FindWithTelegramId,
	) {
		if (isNaN(page) || page <= 0) {
			page = 0;
		}
		...
		return await this.eventService.findParticipate(
			page,
			new FindEventDto(),
			user.id,
		);
	...
	async remove(@Param('id', ParseIntPipe) id: number, @Req() req) {
		...
		if (
			req.user.isAdmin ||
			(req.user.volunteer && req.user.volunteer.id == event.volunteer.id)
		) {
